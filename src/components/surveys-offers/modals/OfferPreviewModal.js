      ...
      <div className="bg-white rounded-lg w-full max-w-md mx-4 max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="bg-gray-900 rounded-3xl p-2 shadow-2xl">
            {/* Mobile Screen */}
            <div className="bg-white rounded-2xl overflow-hidden flex flex-col" style={{ aspectRatio: '9/19.5', maxHeight: '600px' }}>
              {/* Status Bar - Fixed */}
                  ...
                  <div className="p-4 border-b border-gray-100">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        {(isCashback && offer.images?.cardImage) || ((isShopping || isMagicReceipt) && offer.creatives?.images?.["600x300"]) ? (
                          <img
                            src={isCashback ? offer.images.cardImage : offer.creatives.images["600x300"]}
                            alt={offer.title || offer.merchant_name || offer.anchor || offer.product_name}
                            className="w-full h-32 object-cover rounded-lg mb-3"
                            onError={(e) => {
                              e.target.style.display = "none";
                            }}
                          />
                        ) : (isShopping || isMagicReceipt) && offer.creatives?.icon ? (
                          <img
                            src={offer.creatives.icon}
                            alt={offer.title || offer.anchor || offer.product_name}
                        ...
                        <h3 className="font-semibold text-gray-900 text-sm">
                          {offer.title || offer.merchant_name || offer.anchor || offer.product_name || 'Untitled Offer'}
                        </h3>
                        {offer.description && (
                      ...
                      {(isShopping || isMagicReceipt) && offer.total_points && (
                        <div className="ml-3 text-right">
                          <div className="bg-emerald-100 text-emerald-800 px-2 py-1 rounded-full text-xs font-medium">
                            +{parseInt(offer.total_points).toLocaleString()} pts
                          </div>
                        </div>
                      )}
                    </div>
                    ...
                    {isCashback ? (
                      <>
                        ...
                        {offer.merchant_name && (
                          <div className="flex items-center justify-between text-xs">
                            ...
                            <span className="font-medium text-gray-900">{offer.merchant_name}</span>
                          </div>
                        )}
                        {offer.primary_category && (
                          <div className="flex items-center justify-between text-xs">
                            <span className="text-gray-600">Category:</span>
                            <span className="font-medium text-gray-900">{offer.primary_category}</span>
                          </div>
                        )}
                        {offer.reward_delay_days !== undefined && (
                          <div className="flex items-center justify-between text-xs">
                            <span className="text-gray-600">Reward Delay:</span>
                            <span className="font-medium text-gray-900">{offer.reward_delay_days} days</span>
                          </div>
                        )}
                        {offer.country_code && (
                          <div className="flex items-center justify-between text-xs">
                            <span className="text-gray-600">Country:</span>
                            <span className="font-medium text-gray-900">{offer.country_code}</span>
                          </div>
                        )}
                        {offer.flat_payout !== undefined && (
                          <div className="flex items-center justify-between text-xs">
                            <span className="text-gray-600">Flat Payout:</span>
                            <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                              offer.flat_payout ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                            }`}>
                        ...
                        {offer.rank !== undefined && (
                          <div className="flex items-center justify-between text-xs">
                            ...
                            <span className="font-medium text-gray-900">{offer.rank}</span>
                          </div>
                        )}
                          ...
                          <div className="pt-2 border-t border-gray-100">
                            ...
                            <ul className="mt-1 space-y-1">
                              {offer.terms.slice(0, 3).map((term, idx) => (
                                <li key={idx} className="text-xs text-gray-600">• {term}</li>
                              ))}
                            </ul>
                          </div>
                        )}
                      ...
                      <>
                        ...
                        {offer.product_name && (
                          <div className="flex items-center justify-between text-xs">
                            ...
                            <span className="font-medium text-gray-900">{offer.product_name}</span>
                          </div>
                        )}
                        {offer.anchor && (
                          <div className="flex items-center justify-between text-xs">
                            <span className="text-gray-600">Anchor:</span>
                            <span className="font-medium text-gray-900">{offer.anchor}</span>
                          </div>
                        )}
                        {offer.categories && offer.categories.length > 0 && (
                          <div className="flex items-center justify-between text-xs">
                            <span className="text-gray-600">Category:</span>
                            <span className="font-medium text-gray-900">{offer.categories[0]}</span>
                          </div>
                        )}
                        {offer.confirmation_time && (
                          <div className="flex items-center justify-between text-xs">
                            <span className="text-gray-600">Confirmation:</span>
                            <span className="font-medium text-gray-900">{offer.confirmation_time}</span>
                          </div>
                        )}
                        {offer.pending_time && (
                          <div className="flex items-center justify-between text-xs">
                            <span className="text-gray-600">Pending Time:</span>
                        ...
                        {offer.epc && (
                          <div className="flex items-center justify-between text-xs">
                            ...
                            <span className="font-medium text-gray-900">${offer.epc}</span>
                          </div>
                        )}
                        {offer.mobile_verification_required !== undefined && (
                          <div className="flex items-center justify-between text-xs">
                            <span className="text-gray-600">Mobile Verification:</span>
                            <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                              offer.mobile_verification_required ? 'bg-orange-100 text-orange-800' : 'bg-gray-100 text-gray-800'
                            }`}>
                              {offer.mobile_verification_required ? 'Required' : 'Not Required'}
                            </span>
                          </div>
                        )}
                        {offer.is_game !== undefined && (
                          <div className="flex items-center justify-between text-xs">
                            ...
                            <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                              offer.is_game ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'
                            }`}>
                              {offer.is_game ? 'Yes' : 'No'}
                            </span>
                          </div>
                        )}
                        {offer.things_to_know && offer.things_to_know.length > 0 && (
                          <div className="pt-2 border-t border-gray-100 pb-2">
                            <span className="text-xs text-gray-600 font-medium block mb-1">Things to Know:</span>
                            <ul className="space-y-1">
                              {offer.things_to_know.map((item, idx) => (
                                <li key={idx} className="text-xs text-gray-600 leading-relaxed">• {item}</li>
                              ))}
                            </ul>
                          </div>
                        )}
                        {offer.requirements && (
                          <div className="pt-2 border-t border-gray-100 pb-2">
                            <span className="text-xs text-gray-600 font-medium block mb-1">Requirements:</span>
                            <p className="text-xs text-gray-600 leading-relaxed">{offer.requirements}</p>
                          </div>
                        )}
                      </>
                    ) : (
                      <>
                        ...
                        <div className="flex items-center justify-between text-xs">
                          ...
                          <span className="font-medium text-gray-900">{offer.estimatedTime || 0} min</span>
                        </div>
                        <div className="flex items-center justify-between text-xs">
                        ...
                        <div className="flex items-center justify-between text-xs">
                          <span className="text-gray-600">Category:</span>
                          <span className="font-medium text-gray-900">{offer.category || 'General'}</span>
                        </div>
                        <div className="flex items-center justify-between text-xs">
                          <span className="text-gray-600">Powered by:</span>
                          <span className="font-medium text-blue-600">{offer.sdkSource || offer.sdkProvider || 'bitlabs'}</span>
                        </div>
              ...
              {(isCashback && offer.images?.backgroundImage) || ((isShopping || isMagicReceipt) && offer.creatives?.images) ? (
                <div className="px-4 pb-4">
                  <div className="bg-gray-50 border border-gray-200 rounded-lg p-3">
                    <h4 className="font-medium text-gray-900 text-xs mb-2">
                      {isCashback ? 'Merchant Details' : isMagicReceipt ? 'Receipt App Details' : 'Product Images'}
                    </h4>
                    {isCashback && offer.images?.backgroundImage ? (
                      ...
                      />
                    ) : (isShopping || isMagicReceipt) && offer.creatives?.images?.["630x315"] ? (
                      <img
                        src={offer.creatives.images["630x315"]}
                        alt={offer.anchor || offer.product_name || offer.title}
          ...
          <div className="text-xs text-gray-600 mb-3">
            This preview shows how the {isCashback ? 'cashback offer' : isShopping ? 'shopping offer' : isMagicReceipt ? 'magic receipt offer' : 'survey'} will appear to users on mobile devices. 
            {!isCashback && !isShopping && !isMagicReceipt && ` The actual survey content is provided by ${offer.sdkSource || offer.sdkProvider || 'bitlabs'}.`}
          </div>
            {/* Detailed Information Section */}
            <div className="bg-white rounded-lg p-3 mb-3 border border-gray-200">
              <h4 className="font-semibold text-sm text-gray-900 mb-2">All Offer Details</h4>
              <div className="space-y-1 text-xs">
                <div className="flex justify-between">
                  <span className="text-gray-600">Offer ID:</span>
                  <span className="font-mono text-gray-900 break-all">{offer.id || 'N/A'}</span>
                </div>
                <div className="flex justify-between">
                ...
                <>
                  {offer.merchant_id && (
                    <div className="flex justify-between">
                      ...
                      <span className="font-mono text-gray-900">{offer.merchant_id}</span>
                    </div>
                  )}
                  {offer.rank !== undefined && (
                    <div className="flex justify-between">
                  ...
                  {offer.tier_mappings && offer.tier_mappings.length > 0 && (
                    <div className="flex justify-between">
                      ...
                      <span className="text-gray-900">{offer.tier_mappings.length} tier(s)</span>
                    </div>
                  )}
                </>
              )}
              {(isShopping || isMagicReceipt) && (
                <>
                  {offer.product_id && (
                    <div className="flex justify-between">
                      ...
                      <span className="font-mono text-gray-900">{offer.product_id}</span>
                    </div>
                  )}
                  {offer.funnel_id && (
                    <div className="flex justify-between">
                      <span className="text-gray-600">Funnel ID:</span>
                      <span className="font-mono text-gray-900 break-all">{offer.funnel_id}</span>
                    </div>
                  )}
                  {offer.events && offer.events.length > 0 && (
                    <div className="flex flex-col">
                      <span className="text-gray-600 mb-1">Events ({offer.events.length}):</span>
                      <div className="text-xs space-y-1 max-h-24 overflow-y-auto">
                        {offer.events.filter(e => e.payable).slice(0, 3).map((event, idx) => (
                          <div key={idx} className="text-gray-700">
                            • {event.name}: {event.points} pts (${event.payout})
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {offer.session_hours && (
                    <div className="flex justify-between">
                      ...
                      <span className="text-gray-900">{offer.session_hours} hours</span>
                    </div>
                  )}
                  {offer.disclaimer && (
                    <div className="flex flex-col">
                      <span className="text-gray-600 mb-1">Disclaimer:</span>
                      <span className="text-xs text-gray-700 break-words">{offer.disclaimer.substring(0, 200)}...</span>
                    </div>
                  )}
                </>
              )}
                {offer.clickUrl && (
                  <div className="flex flex-col">
                    ...
                    <span className="font-mono text-gray-900 text-xs break-all" title={offer.clickUrl}>
                      {offer.clickUrl}
                    </span>
                  </div>
                )}
                <div className="flex justify-between">
                  ...
                  <span className="text-gray-900">{offer.provider || offer.sdkProvider || 'bitlabs'}</span>
                </div>
              </div>
            </div>
          </div>
