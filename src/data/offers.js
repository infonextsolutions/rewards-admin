...
export const offersAPI = {
    ...
    try {
      ...
      const apiPayload = {
        ...
        description:
          ...
          `Complete ${offerData.offerName} to earn rewards`,
        sdkProvider: offerData.sdkProvider || "bitlabs",
        tierAccess: (offerData.tiers || []).map((tier) => tier.toLowerCase()),
        countries:
          offerData.countries || offerData.segments?.country
            ? [offerData.segments.country]
            : [],
        expiryDate: offerData.endDate
          ? new Date(offerData.endDate).toISOString()
          : offerData.expiryDate,
        isAdSupported:
          offerData.adOffer === "Ad-Based" || offerData.isAdSupported || false,
        xptrRule: offerData.xpTier || offerData.xptrRule || "",
        reward: {
          coins:
            offerData.rewardType === "Coins"
              ? parseInt(offerData.rewardValue) || 0
              : 0,
          xp:
            offerData.rewardType === "XP"
              ? parseInt(offerData.rewardValue) || 0
              : 0,
        },
        metadata: {
          estimatedTimeMinutes: offerData.estimatedTime
            ? parseInt(offerData.estimatedTime)
            : 10,
          difficulty: offerData.difficulty
            ? offerData.difficulty.toLowerCase()
            : "medium",
          category: offerData.category
            ? offerData.category.toLowerCase()
            : "survey",
      ...
      return {
        ...
        estimatedTime: offer.metadata?.estimatedTimeMinutes
          ? `${offer.metadata.estimatedTimeMinutes} min`
          : "N/A",
        difficulty: offer.metadata?.difficulty
          ? offer.metadata.difficulty.charAt(0).toUpperCase() +
            offer.metadata.difficulty.slice(1)
          : "Medium",
        category: offer.metadata?.category
          ? offer.metadata.category.charAt(0).toUpperCase() +
            offer.metadata.category.slice(1)
          : "General",
  ...
  async updateOffer(offerId, offerData) {
    try {
      ...
      const response = await apiClient.put(
        `/admin/game-offers/offers/${offerId}`,
        apiPayload
      );
  ...
  async getOffers(params = {}) {
    try {
      ...
      const response = await apiClient.get(
        `/admin/game-offers/offers?${queryParams.toString()}`
      );
